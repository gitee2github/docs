...
| **Параметр**                        | **Описание**                                                 |
| ----------------------------------- | ------------------------------------------------------------ |
| -bp *файл spec*                     | Запустить только **%prep** *файла spec* ( распаковать исходный код и установить патч). |
| -bc *файл spec*                     | Выполните разделы **%prep** и **%build** *файла spec* ( подготовка и компиляция). |
| -bi *файл spec*                     | Выполнение спецификации в **%prep** , **%build** и **%install** ( подготовка, компиляция и установка). |
| -bl *файл spec*                     | Проверьтераздел **%file** *файла spec*.     |
