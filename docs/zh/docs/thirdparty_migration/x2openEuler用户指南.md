...
-   [介绍](#介绍)
    -   [免责声明](#免责声明)
    -   [x2openEuler原地升级工具简介](#x2openeuler-原地升级工具简介)
    -   [应用场景](#应用场景)
    -   [部署方式](#部署方式)
    -   [访问和使用](#访问和使用)
        -   [界面说明](#界面说明)
        -   [登录x2openEuler工具web界面](#登录x2openeuler工具web界面)
        -   [使用前配置](#使用前配置)
...
-   [特性指南](#特性指南)
    ...
    -   [通过web界面使用x2openEuler工具](#通过web界面使用x2openeuler工具)
        -   [升级任务](#升级任务)
            -   [创建升级任务](#创建升级任务)
            -   [进行节点升级](#进行节点升级)

    -   [通过CLI使用x2openEuler工具](#通过cli使用x2openeuler工具)
        -   [使用x2openEuler工具进行原地升级](#使用x2openeuler工具进行原地升级)
        -   [升级后验证](#升级后验证)

-   [常用操作](#常用操作)
    -   [（可选）为x2openEuler工具导入根证书](#可选为x2openeuler工具导入根证书)
    -   [关闭SELinux](#关闭selinux)
    ...
    -   [管理用户](#管理用户)
        -   [重置密码](#重置密码)
        -   [删除用户](#删除用户)
        -   [修改当前登录用户密码](#修改当前登录用户密码)
    -   [管理弱口令字典](#管理弱口令字典)
    -   [管理web服务端证书](#管理web服务端证书)
    -   [查询工具发布信息](#查询工具发布信息)
    -   [查看工具免责声明](#查看工具免责声明)
    -   [版本查看(CLI)](#版本查看cli)

-   [FAQ](#faq)
    -   [CLI模式下升级失败后回滚](#cli模式下升级失败后回滚)
    -   [x2openEuler升级失败-救援模式](#x2openeuler升级失败-救援模式)
    -   [升级后部分命令无法执行](#升级后部分命令无法执行)   
    -   [升级失败-DNF升级报错](#升级失败-dnf升级报错)
    -   [升级过程漫长](#升级过程漫长)
    -   [升级前检查grub2版本不满足要求](#升级前检查grub2版本不满足要求)
...
>-   确定硬件是否兼容新的操作系统。
>-   确定当前环境已操作过的相关配置能否可以继承到新操作系统。

##### 操作步骤

1. 使SSH远程登录工具以root用户登录至需要升级的环境操作系统命令行界面，此处以在x86平台下将CentOS7.6升级至openEuler 20.03-LTS-SP1系统为例。
...
2. <a name="li536331718423"></a>（可选）执行以下命令修改x2openEuler工具升级脚本。
   ...
   ```
   vi /usr/bin/centos2openEuler
   ```

   - 对以下配置内容进行修改，实现修改升级的目标操作系统repo源。

     ```
     [openEuler-2003-lts-sp1]
     name=openEuler-2003-lts-sp1
     baseurl=http://mirrors.tools.huawei.com/openeuler/openEuler-20.03-LTS-SP1/everything/x86_64/
     enabled=1
     gpgcheck=0
     
     [openEuler-2003-lts-sp1-update]
     name=openEuler-2003-lts-sp1-update
     baseurl=http://mirrors.tools.huawei.com/openeuler/openEuler-20.03-LTS-SP1/update/x86_64/
     enabled=1
     gpgcheck=0
     ```

   - 对以下配置部分进行修改，实现修改"/etc/sut/sut.conf"

     ```
     function modify_conf()
     {
         sed -i "s#^repo_source.*#repo_source = other#g" /etc/sut/sut.conf
         sed -i "s#^check_level.*#check_level = high_impact#g" /etc/sut/sut.conf
         sed -i "s#^conflict_rpm.*#conflict_rpm = libstorage-uio-static.aarch64;centos-release#g" /etc/sut/sut.conf
     }
     ```

     **表 14**  参数描述

     <a name="table201731811191111"></a>

     <table><thead align="left"><tr id="row1317481114111"><th class="cellrowborder" valign="top" width="15.909999999999998%" id="mcps1.2.4.1.1"><p id="p1117461171112"><a name="p1117461171112"></a><a name="p1117461171112"></a>参数</p>
     </th>
     <th class="cellrowborder" valign="top" width="14.97%" id="mcps1.2.4.1.2"><p id="p1344184110116"><a name="p1344184110116"></a><a name="p1344184110116"></a>参数值</p>
     </th>
     <th class="cellrowborder" valign="top" width="69.12%" id="mcps1.2.4.1.3"><p id="p7174181171119"><a name="p7174181171119"></a><a name="p7174181171119"></a>描述</p>
     </th>
     </tr>
     </thead>
     <tbody><tr id="row917411113117"><td class="cellrowborder" valign="top" width="15.909999999999998%" headers="mcps1.2.4.1.1 "><p id="p01772113117"><a name="p01772113117"></a><a name="p01772113117"></a>repo_source</p>
     </td>
     <td class="cellrowborder" valign="top" width="14.97%" headers="mcps1.2.4.1.2 "><a name="ul1745203361518"></a><a name="ul1745203361518"></a><ul id="ul1745203361518"><li>iso</li><li>other</li></ul>
     </td>
     <td class="cellrowborder" valign="top" width="69.12%" headers="mcps1.2.4.1.3 "><p id="p13177161121116"><a name="p13177161121116"></a><a name="p13177161121116"></a>默认值为other</p>
     <a name="ul203961245141819"></a><a name="ul203961245141819"></a><ul id="ul203961245141819"><li>当取iso时，无需手动配置repo源，只需将升级目标系统的iso文件上传至系统的/root目录下，执行升级时，使用-p参数指定即可自动创建本地源。</li><li>当取other时，需手动配置repo源，此源为升级目标系统的repo源。</li></ul>
     </td>
     </tr>
     <tr id="row917715111112"><td class="cellrowborder" valign="top" width="15.909999999999998%" headers="mcps1.2.4.1.1 "><p id="p4177111181114"><a name="p4177111181114"></a><a name="p4177111181114"></a>check_level</p>
     </td>
     <td class="cellrowborder" valign="top" width="14.97%" headers="mcps1.2.4.1.2 "><a name="ul1234526161615"></a><a name="ul1234526161615"></a><ul id="ul1234526161615"><li>low_impact</li><li>high_impact</li></ul>
     </td>
     <td class="cellrowborder" valign="top" width="69.12%" headers="mcps1.2.4.1.3 "><p id="p6177181116113"><a name="p6177181116113"></a><a name="p6177181116113"></a>默认值为 high_impact</p>
     <a name="ul1933584952119"></a><a name="ul1933584952119"></a><ul id="ul1933584952119"><li>当repo_source参数值为other时，check_level参数值为high_impact。</li><li>当repo_source参数值为iso时，check_level参数值为low_impact。</li></ul>
     </td>
     </tr>
     <tr id="row1217710118111"><td class="cellrowborder" valign="top" width="15.909999999999998%" headers="mcps1.2.4.1.1 "><p id="p1585744141420"><a name="p1585744141420"></a><a name="p1585744141420"></a>conflict_rpm</p>
     </td>
     <td class="cellrowborder" valign="top" width="14.97%" headers="mcps1.2.4.1.2 "><p id="p16451941121118"><a name="p16451941121118"></a><a name="p16451941121118"></a>-</p>
     </td>
     <td class="cellrowborder" valign="top" width="69.12%" headers="mcps1.2.4.1.3 "><p id="p117701121114"><a name="p117701121114"></a><a name="p117701121114"></a>当进行升级过程中出现某个包有冲突的问题，需将其添加至此处进行解决。</p>
     </td>
     </tr>
     </tbody>
     </table>
     ...
     >
     >-   **repo\_source = other**
     >-   **check\_level = high\_impact**

   - 对以下配置内容进行修改，配置升级完成后需要替换的包。

     ```
     function swap_clear()
     {
     # swap vendor pkgs
         sed -i "s#^source_rpm =.*#source_rpm = centos-logos centos-release#g" /etc/sut/sut.conf
         sed -i "s#^target_rpm =.*#target_rpm = openEuler-logos openEuler-release#g" /etc/sut/sut.conf
         x2openEuler-upgrade swap -y
     if [ $? -ne 0 ]; then
     echo"x2openEuler-upgrade swap failed, please check."
     exit 1
     fi
     return 0
     }
   ...
   - 对以下配置内容进行修改，删除升级完成后遗留的部分CentOS安装包。

     若待升级系统非CentOS 7.6，则需确认待升级系统中RPM的dist后选择是否移除升级完成后遗留的安装包，此处以待升级系统为openEuler为例。

     1. 执行以下命令获取RPM包的dist。

        ```
        rpm -q --qf %{release} systemd
        ```

        回显信息如下，其中“oel”为openEuler的RPM包dist。

        ```
        50.oel
        ```

     2. 若需要移除升级完成后遗留的安装包，则需修改以下配置内容中的“pkg\_list=$\(rpm -qa | grep el7\)“中“el7”为“oel”。

        ```
        function clear_el7_pkg()
        {
        # after migrate, we still have centos pkgs remain
        # we can remove this pkgs
        echo"remove residual rpms"
            pkg_list=$(rpm -qa | grep el7)
        forpkgin${pkg_list[@]};
        do
        echo"remove rpm ${pkg}"
                rpm -e $pkg --nodeps
        if [ $? -ne 0 ];then
        echo"uninstall rpm ${pkg} failed"
        exit 1
        fi
        done
        echo"remove residual rpms done."
        }
        ```

     3. 若需要保留遗留的安装包，则可选择删除配置内容“function clear\_el7\_pkg\(\)“或注释函数名以停止调用该功能。
...
3. 为确保升级过程正常，需进行以下内容的升级前检查。

   -   确保"/etc/yum.repos.d"下的repo源只有[2](#li536331718423)配置的**openEuler-2003-lts-sp1**与**openEuler-2003-lts-sp1-update**仓库为启用状态，其他仓库需全部禁用。
   -   确保待升级环境能够访问"https://repo.openeuler.org/"。
...
4. 使用x2openEuler工具进行升级操作。
   ...
   >-   再次使用centos2openEuler升级脚本前，需要将“/etc/yum.repos.d/migrate-backup“目录下备份的repo文件还原到“/etc/yum.repos.d/“目录下，将其它repo文件全部移除。

   执行以下命令，对待升级操作系统进行升级。

   ```
   bash -x centos2openeuler
   ```
