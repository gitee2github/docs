...
例如设置polling时间为400000ns，使用root用户执行命令如下：
...
相比传统的4K内存分页，openEuler也支持2MB/1GB的大内存分页。内存大页可以有效减少TLB(Translation Lookaside Buffer) miss，显著提升内存访问密集型业务的性能。openEuler使用两种技术来实现内存大页。
