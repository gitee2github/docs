...
4.  使用dd命令将ISO安装镜像直接写入USB盘：
    ...
    例如：如果该ISO镜像文件位于 /home/testuser/Downloads/openEuler-20.03-LTS-SP1-aarch64-dvd.iso，同时探测到的设备名称为sdb，则该命令如下：
...
2.  使用qemu-img工具的create命令，创建镜像文件，命令格式为：
    ...
    例如，创建一个磁盘设备大小为32GiB、格式为qcow2的镜像文件openEuler-image.qcow2，命令和回显如下：
...
各步骤详细的操作请参考《[openEuler 20.03 LTS SP1 虚拟化用户指南](./../Virtualization/virtualization.html)》。
