...
Assume that you want to identify which time zone is the closest to your present location while you are in Asia, you can check that by listing all available time zones in Asia with the following command:
...
To change the current date, run the command with the **--set** or **-s** command line option as the **root** user. In the command,  _YYYY_  indicates the year,  _MM_  indicates the month, and  _DD_  indicates the day. Change them as required.
