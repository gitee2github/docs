...
* The repo source is officially released at: <https://repo.openeuler.org/openEuler-21.03/everything/$basearch/>
* You can obtain the source code at: <https://gitee.com/openeuler/pkgship>
* You can obtain the daily buid at: <https://build.openeuler.org/package/show/openEuler:21.03/pkgship>
...
Use DNF to mount the repo source where the pkgship is located (for details, see the [Application Development Guide](../ApplicationDev/application-development.md). Then run the following command to download and install the pkgship and its dependencies:
...
>**说明:**
> Elasticsearch installed using an RPM package is in password-free mode by default, which is required by pkgship. Therefore, you are advised to install Elasticsearch and pkgship on the same server to so that they are isolated from external networks for security.
...
; If pkgship is deployed in the client-server mode, query_ip_addr of the server must be the IP address of the local host or 0.0.0.0.
; The client can access the server using query_ip_addr and query_port, or the mapped remote_host.
...
[DATABASE-Database]
; Database access address. You are advised to use the IP address of the local host.
...
Path: **/var/log/pkgship/log_info.log**. You can customize the path through the **log\_path** field in the **package.ini** file.
...
Path: **/var/log/pkgship/log_info.log**. You can customize the path through the **daemonize** field in the **package.ini** file.
